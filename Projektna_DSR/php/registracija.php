...
<?php
   include_once('header.php');
       ...
       require_once('connection.php');
		...
		#*********************************************************************************/
		$poizvedba = $povezava->prepare('SELECT email FROM registriran_uporabnik'); //SPREMENI POIZVEDBO GLEDE NA PODATKOVNO BAZO
	// DEJANSKA IZVEDBA POIZVEDBE
		...
		&& isset($_POST['geslo']))
		{
			...
			$pdoQuery=("INSERT INTO  registriran_uporabnik(`uporabnisko_ime`, `email`,`geslo`) VALUE (:uporabnisko_ime,:email, :geslo )"); //SPREMENI POIZVEDBO GLEDE NA PODATKOVNO BAZO
		
			$pdoRezultat= $povezava->prepare($pdoQuery);

			 $pdoRezultat->execute(array(":uporabnisko_ime"=>$uporabnisko_ime, ":email"=> $email, ":geslo"=>$geslo));
